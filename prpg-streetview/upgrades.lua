...
addEventHandler("streetview->getWorkUpgrades", resourceRoot, function()
    triggerClientEvent(client, "streetview->getWorkUpgrades", resourceRoot, upgrades)
end)

addEvent("streetview->getMyWorkInfo", true)
addEventHandler("streetview->getMyWorkInfo", resourceRoot, function()
    ...
    triggerClientEvent(client, "streetview->getMyWorkInfo", resourceRoot, fromJSON(result[1].upgrades), result[1].ppoints)
end)

addEvent("streetview->buyWorkUpgrade", true)
addEventHandler("streetview->buyWorkUpgrade", resourceRoot, function(index)
    ...
    if result[1] then
        ...
        local exec = exports["mysql"]:exec("Update prpg_streetview_upgrades set upgrades = ?, ppoints = ppoints - ? where sid = ? LIMIT 1;", toJSON(myUpgrades), upgradeToBuy.cost, sid)
        if exec then
            exports["prpg-notifications"]:newNotification("success", "Pomyślnie aktywowano ulepszenie " .. upgradeToBuy.title .. ".", client)
            triggerClientEvent(client, "streetview->getMyWorkInfo", resourceRoot, myUpgrades, result[1].ppoints - upgradeToBuy.cost)
        end
    else
...
addEventHandler("streetview->getWorkRanking", resourceRoot, function()
    local sid = getElementData(client, "player:sid")
    if not sid then return end

    local result = exports["mysql"]:query("Select X.login, X.id, Y.ppoints from prpg_users X, prpg_streetview_upgrades Y where X.id = Y.sid order by ppoints desc LIMIT 3;");
    triggerClientEvent(client, "streetview->getWorkRanking", resourceRoot, result)
end)