...
renderGui = function()
    ...
    if settings.selected == "Strona główna" then
        ...
        dxDrawText("Wymagania: 3500 SRP, Ciężarówka", gui_positions.text.x, gui_positions.text.y, gui_positions.text.w + gui_positions.text.x, gui_positions.text.h + gui_positions.text.y, white, 1.00, fonts.roboto, "center", "center")
        ...
        local color = settings.working and tocolor(200, 0, 0, hover) or tocolor(33, 84, 229, hover)
        local text = settings.working and "Zakończ" or "Rozpocznij"
        dxDrawImage(gui_positions.button.x, gui_positions.button.y, gui_positions.button.w, gui_positions.button.h, textures.button, 0, 0, 0, color)
        dxDrawText(text, gui_positions.button.x, gui_positions.button.y, gui_positions.button.w + gui_positions.button.x, gui_positions.button.h + gui_positions.button.y, white, 1.00, fonts.roboto, "center", "center")
    end
...
startWorkButtonClick = function(b, s)
    if b == "left" and s == "up" then
        ...
        if isMouseInPosition(gui_positions.button.x, gui_positions.button.y, gui_positions.button.w, gui_positions.button.h) then
            triggerServerEvent("trucker->startWork", resourceRoot)
            triggerServerEvent("trucker->isPlayerWorking", resourceRoot)
        end
    end
end
        ...
        for i, v in ipairs(upgrades) do
            if i >= settings.site and i <= settings.column then
                ...
                if isMouseInPosition(gui_positions.has.x, gui_positions.has.y + offsetY, gui_positions.has.w, gui_positions.has.h) then
                    triggerServerEvent("trucker->buyWorkUpgrade", resourceRoot, i)
...
addEventHandler("trucker->getWorkUpgrades", resourceRoot, getWorkUpgrades)
...
addEventHandler("trucker->getMyWorkInfo", resourceRoot, getMyWorkInfo)
...
addEventHandler("trucker->getWorkRanking", resourceRoot, getWorkRanking)


isPlayerWorking = function(bool)
    settings.working = bool
end
addEvent("trucker->isPlayerWorking", true)
addEventHandler("trucker->isPlayerWorking", resourceRoot, isPlayerWorking)


checkPlayerVehicle = function(allowedList)
    checkIfVehicleIsTruck = function(player)
        if not allowedList[getElementModel(source)] then
            exports["prpg-notifications"]:newNotification("error", "Ten pojazd nie jest ciężarówką. Praca zakończona.")
            triggerServerEvent("trucker->stopWork", resourceRoot)
        end
        removeEventHandler("onClientVehicleEnter", root, checkIfVehicleIsTruck)
    end
    addEventHandler("onClientVehicleEnter", root, checkIfVehicleIsTruck)
end
addEvent("trucker->checkPlayerVehicle", true)
addEventHandler("trucker->checkPlayerVehicle", resourceRoot, checkPlayerVehicle)
...
showGui = function(bool)
    if bool == true then
        ...
        triggerServerEvent("trucker->getWorkUpgrades", resourceRoot)
        triggerServerEvent("trucker->getMyWorkInfo", resourceRoot)
        triggerServerEvent("trucker->getWorkRanking", resourceRoot)
        triggerServerEvent("trucker->isPlayerWorking", resourceRoot)
...
addEvent("trucker->showGui", true)
addEventHandler("trucker->showGui", resourceRoot, showGui)